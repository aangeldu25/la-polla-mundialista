...
export function matchToIcsEvent(match: Match, sequence = 0): string {
  ...
  const lines: (string | null)[] = [
    "BEGIN:VEVENT",
    `UID:pmfu-match-${match.id}@polla-mundialista-familia-unida.vercel.app`,
...
export function matchesToIcs(matches: Match[]): string {
  const header = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//PMFU//Mundial 2026//ES",
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH",
    "X-WR-CALNAME:Mundial 2026 · PMFU",
    "X-WR-CALDESC:Calendario de partidos del Mundial 2026 — La Polla Mundialista",
